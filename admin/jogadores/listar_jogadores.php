<?php
	...
	if(empty($busca_v)) {
			$result_Jogadores = "SELECT * FROM Jogadores ORDER BY id DESC LIMIT $inicio, $qnt_result_pg";
			$resultado_Jogadores = mysqli_query($conexao, $result_Jogadores);
	}
	else {
			$result_Jogadores = "SELECT * FROM Jogadores WHERE nome LIKE '%$busca_v%' ORDER BY id DESC LIMIT $inicio, $qnt_result_pg";
			$resultado_Jogadores = mysqli_query($conexao, $result_Jogadores);
	}

	//Verificar se encontrou resultado na tabela "Jogadores"
	if(($resultado_Jogadores) AND ($resultado_Jogadores->num_rows != 0)){
		...
		<table class="table table-striped table-bordered table-hover container-lg">
			...
			<tbody>
				<?php
				while($row_Jogadores = mysqli_fetch_array($resultado_Jogadores)){
					?>
					<tr>
						<th><?php echo $row_Jogadores['id']; ?></th>
                        <td class="col-9"><?php echo $row_Jogadores['nome']; ?></td>
                        <td class="col-9"><?php echo $row_Jogadores['posicao']; ?></td>
                        <td class="col-9"><?php echo $row_Jogadores['selecao']; ?></td>
						<td class="col-1"><?php echo $row_Jogadores['dtCadastro']; ?></td>
						<td class="text-center"><a class="btn btn-primary" href='jogadores/atualizar_jogador.php?idj=<?=$row_Jogadores['id']?>'> Alterar </a></td>
                        <td class="text-center"><a class="btn btn-danger" href='../../php/jogadores/delete-jogador.php?idj=<?=$row_Jogadores['id']?>' 
		...
		$result_pg = "SELECT COUNT(id) AS num_result FROM Jogadores";
		...
		echo "<div class='container alert alert-danger' role='alert'> Jogador não encontrado!</div>";
	}
