    ...
    if(isset($_SESSION['id']) && !empty($_SESSION['id'])) {
        ...
        if($exibe_nivel['usuario'] != 'admin') {
          echo 'Acesso negado';
            ...
            if(!empty($id)) {
                mysqli_query($conexao, "DELETE FROM Posicoes WHERE id = ".$id);

                if(mysqli_affected_rows($conexao)) {
                    $_SESSION['msg'] = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                        <i class="bx bx-check" ></i> Posição apagada.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
                    header("location:../../admin/posicoes.php");
                }
                else {
                    
                    $_SESSION['msg'] = '<div class="alert alert-danger" role="alert">
                        <i class="bx bx-error-circle" ></i> Erro: a posição não foi apagada.</div>';
                    header("location:../../admin/posicoes.php");
                }
            } 
            else {	
                $_SESSION['msg'] = '<div class="alert alert-warning" role="alert">
                    <i class="bx bx-error-circle" ></i> Nenhuma posição foi selecionada.</p>';
                header("location:../../admin/posicoes.php");
            }
        }
    }
